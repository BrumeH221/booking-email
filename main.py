...
#  12. (Google Sheets mirror is auto-triggered by save_booking)
...
def process_booking_emails():
    ...
    if not emails:
        ...
        return {"found": 0, "processed": 0, "skipped_duplicate": 0,
                "not_relevant": 0, "errors": 0}

    print(f"Found {len(emails)} unread email(s).")
    summary = {"found": len(emails), "processed": 0, "skipped_duplicate": 0,
               "not_relevant": 0, "errors": 0}
...
def _process_one_email(message_id):
    ...
    # Per business rule: touch NO database tables at all (no `bookings` row,
    # no `processed_emails` row). Only mark the email as read in Gmail so
    # it doesn't appear in the next unread-search.
    ...
    if similar and similar[0]["score"] >= config.DUPLICATE_BLOCK_THRESHOLD:
        dup = similar[0]
        # Same sender + very high similarity = same person sending the same
        # request twice. Block save, no reply, just mark as read.
    ...
    # ---- 12) Sheets mirror auto via save_booking ----
...
def _decide_status(intent, info):
    ...
    # Price, complaint, general question — always Pending for manager review
    return config.STATUS_PENDING
...
def _send_ack_email(detail, record):
    ...
    if record["status"] == config.STATUS_NEED_MORE_INFO:
        ...
        return  # no auto-reply for not_relevant
